...
if( is_admin() ) {
	...
	function woo_ce_categories_filter_by_language() {
		...
		ob_start(); ?>
<p><label><input type="checkbox" id="categories-filters-language" /> <?php _e( 'Filter Categories by Language', 'woo_ce' ); ?></label></p>
<div id="export-categories-filters-language" class="separator">
	<ul>
		<li>
<?php if( !empty( $languages ) ) { ?>
			<select data-placeholder="<?php _e( 'Choose a Language...', 'woo_ce' ); ?>" name="category_filter_language[]" multiple class="chzn-select" style="width:95%;">
...
<?php } ?>
	...
	<p class="description"><?php _e( 'Select the Language\'s you want to filter exported Categories by. Default is to include all Language\'s.', 'woo_ce' ); ?></p>
</div>
...
function woo_ce_get_category_fields( $format = 'full' ) {
	...
	$fields[] = array(
		...
		'label' => __( 'Category: Level 1', 'woo_ce' )
	);
	$fields[] = array(
		'name' => 'category_level_2',
		'label' => __( 'Category: Level 2', 'woo_ce' )
	);
	$fields[] = array(
		'name' => 'category_level_3',
		'label' => __( 'Category: Level 3', 'woo_ce' )
	);
	$fields[] = array(
		'name' => 'description',
